...
//   console.log(superMarket.length);
document.write("<h1> MIX - ARRAY </h1> ");
// 1. Filter the elements of type: ‘dairy’ and calculate averageRatings
let typeDairy = superMarket.filter((Dairy) => Dairy.type === "dairy");
// console.log(typeDairy);
document.write(
  `<h2><u>The elements with type 'dairy' and calculate averageRatings</u></h2>`
);
let avgRating = typeDairy.reduce((acc, Rating) => acc + Rating.rating, 0);
let avgRate = avgRating / typeDairy.length;
document.write(`<b>The average rating For the Dairy's is : </b>${avgRate}`);
console.log(avgRate);
// 2. Filter the elements of type: ‘vegetable’ and calculate averagePrice
let typeVegetable = superMarket.filter(
  (vegetable) => vegetable.type === "vegetable"
);
document.write(
  `<h2><u>The elements with type : ‘vegetable’ and calculate averagePrice</u></h2>`
);
let avgPrice = typeVegetable.reduce((acc, price) => acc + price.price, 0);
let avgPri = avgPrice / typeVegetable.length;
document.write(
  `<b>The average Price For the vegetables is : </b>${Math.ceil(avgPri)}`
);
// 3. Filter the elements of type: ‘fruit’ and add new property ‘isAvailable: true’
let typeFruit = superMarket.filter((fruit) => fruit.type === "fruit");
document.write(
  `<h2><u>The elements of type: ‘fruit’ and add new property ‘isAvailable: true’ </u></h2>`
);
let addTrue = typeFruit.map((add) => {
  return {
    ...add,
    isAvailable: true,
  };
...
let typeDairyTrue = superMarket.filter(
  (Dairy) => Dairy.type === "dairy" || Dairy.type === "vegetable"
);

document.write(
  `<h2><u>The elements of type: 'Dairy' and add new property ‘isAvailable: true’ </u></h2>`
);
let addTrueDairy = typeDairyTrue.map((add) => {
  return {
    ...add,
    isAvailable: true,
  };
});
console.log(addTrueDairy);
addTrueDairy.forEach((valuesDairy) => {
  for (const [key, value] of Object.entries(valuesDairy)) {
    document.write(`<b>${key}</b> : ${value}<br>`);
  }
  document.write("<hr><br>");
});
//vegetable False
let typeVegetableFalse = superMarket.filter(
  (vegetable) => vegetable.type === "vegetable"
);
document.write(
  `<h2><u>The elements of type: 'vegetable' and add new property ‘isAvailable: false' </u></h2>`
);
let addFalseVegetable = typeVegetableFalse.map((add) => {
  return {
    ...add,
    isAvailable: false,
  };
});
console.log(addFalseVegetable);
addFalseVegetable.forEach((valuesVeg) => {
  for (const [key, value] of Object.entries(valuesVeg)) {
    document.write(`<b>${key}</b> : ${value}<br>`);
  }
  document.write("<hr><br>");
});
